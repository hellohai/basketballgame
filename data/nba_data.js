/* Courtside Capital — Real NBA roster snapshot.
 *
 * Player entries: [name, position, age, overallRating]
 * Ratings are editorial estimates for game balance — NOT official NBA data.
 * Refresh with: node scripts/update-rosters.mjs
 */

const NBA_DATA = {
  asOf: '2026-09-21 (via scripts/update-rosters.mjs)',
  teams: [
    { name: 'Atlanta Hawks', abbr: 'ATL', players: [
      ['Jalen Johnson', 'SF', 24, 85],
      ['Dyson Daniels', 'PG', 23, 82],
      ['Onyeka Okongwu', 'SF', 25, 80],
      ['Luguentz Dort', 'PG', 27, 76],
      ['CJ McCollum', 'PG', 35, 76],
      ['Buddy Hield', 'PG', 33, 75],
      ['Nickeil Alexander-Walker', 'PG', 28, 74],
      ['Jock Landale', 'C', 30, 74],
      ['Aaron Wiggins', 'PG', 27, 74],
      ['Corey Kispert', 'SF', 27, 72],
      ['Mouhamed Gueye', 'SF', 23, 71],
      ['Jalen Wilson', 'SF', 25, 71],
    ]},
    { name: 'Boston Celtics', abbr: 'BOS', players: [
      ['Jayson Tatum', 'SF', 28, 93],
      ['Derrick White', 'PG', 32, 85],
      ['Paul George', 'SF', 36, 82],
      ['Payton Pritchard', 'PG', 28, 82],
      ['Mitchell Robinson', 'C', 28, 76],
      ['Sam Hauser', 'SF', 28, 75],
      ['Neemias Queta', 'C', 27, 74],
      ['Mike Conley', 'PG', 38, 72],
      ['Ron Harper Jr.', 'PG', 26, 72],
      ['Baylor Scheierman', 'PG', 25, 72],
      ['Jordan Walsh', 'PG', 22, 71],
      ['Luka Garza', 'C', 27, 70],
    ]},
    { name: 'Brooklyn Nets', abbr: 'BKN', players: [
      ['Michael Porter Jr.', 'SF', 28, 84],
      ['Julius Randle', 'SF', 31, 82],
      ['Terance Mann', 'PG', 29, 74],
      ['Day\'Ron Sharpe', 'C', 24, 74],
      ['Moritz Wagner', 'SF', 29, 74],
      ['Noah Clowney', 'SF', 22, 72],
      ['Egor Demin', 'PG', 20, 72],
      ['Keon Ellis', 'PG', 26, 72],
      ['E.J. Liddell', 'SF', 25, 71],
      ['Josh Minott', 'SF', 23, 70],
      ['Tyson Etienne', 'PG', 27, 69],
      ['Chaney Johnson', 'SF', 24, 68],
    ]},
    { name: 'Charlotte Hornets', abbr: 'CHA', players: [
      ['Brandon Miller', 'SF', 23, 84],
      ['Coby White', 'PG', 26, 82],
      ['Naz Reid', 'C', 27, 79],
      ['Kon Knueppel', 'PG', 21, 76],
      ['Grayson Allen', 'PG', 30, 74],
      ['Dorian Finney-Smith', 'SF', 33, 74],
      ['Dennis Schroder', 'PG', 33, 74],
      ['Grant Williams', 'SF', 27, 74],
      ['Pat Connaughton', 'PG', 33, 73],
      ['Royce O\'Neale', 'SF', 33, 73],
      ['Moussa Diabate', 'SF', 24, 72],
      ['Ryan Kalkbrenner', 'C', 24, 70],
    ]},
    { name: 'Chicago Bulls', abbr: 'CHI', players: [
      ['Josh Giddey', 'PG', 23, 84],
      ['Norman Powell', 'PG', 33, 81],
      ['Nic Claxton', 'C', 27, 80],
      ['Matas Buzelis', 'SF', 21, 79],
      ['Zach Collins', 'SF', 28, 74],
      ['Isaac Okoro', 'SF', 25, 74],
      ['Patrick Williams', 'SF', 25, 74],
      ['Guerschon Yabusele', 'SF', 30, 74],
      ['Rob Dillingham', 'PG', 21, 73],
      ['Tre Jones', 'PG', 26, 73],
      ['Mac McClung', 'PG', 27, 72],
      ['Jalen Smith', 'SF', 26, 72],
    ]},
    { name: 'Cleveland Cavaliers', abbr: 'CLE', players: [
      ['Donovan Mitchell', 'PG', 30, 92],
      ['Evan Mobley', 'C', 25, 89],
      ['James Harden', 'PG', 37, 85],
      ['Jarrett Allen', 'C', 28, 84],
      ['Thomas Bryant', 'C', 29, 74],
      ['Mario Hezonja', 'SF', 31, 74],
      ['Peyton Watson', 'PG', 24, 74],
      ['Sam Merrill', 'PG', 30, 73],
      ['Olivier Sarr', 'SF', 27, 72],
      ['Craig Porter Jr.', 'PG', 26, 71],
      ['Riley Minix', 'SF', 25, 69],
      ['Nae\'Qwan Tomlin', 'SF', 25, 69],
    ]},
    { name: 'Dallas Mavericks', abbr: 'DAL', players: [
      ['Kyrie Irving', 'PG', 34, 88],
      ['Cooper Flagg', 'SF', 19, 84],
      ['Dereck Lively II', 'C', 22, 79],
      ['P.J. Washington', 'SF', 28, 79],
      ['Daniel Gafford', 'SF', 27, 78],
      ['Zaccharie Risacher', 'SF', 21, 78],
      ['Santi Aldama', 'SF', 25, 76],
      ['Naji Marshall', 'SF', 28, 74],
      ['Caleb Martin', 'SF', 30, 74],
      ['Max Christie', 'PG', 23, 72],
      ['Jett Howard', 'PG', 23, 71],
      ['Marcus Sasser', 'PG', 25, 71],
    ]},
    { name: 'Denver Nuggets', abbr: 'DEN', players: [
      ['Nikola Jokic', 'C', 31, 98],
      ['Jamal Murray', 'PG', 29, 87],
      ['Aaron Gordon', 'SF', 31, 82],
      ['Cameron Johnson', 'SF', 30, 80],
      ['Christian Braun', 'PG', 25, 78],
      ['Marvin Bagley III', 'SF', 27, 74],
      ['Tyus Jones', 'PG', 30, 74],
      ['Zeke Nnaji', 'SF', 25, 74],
      ['Lonnie Walker IV', 'PG', 27, 74],
      ['David Roddy', 'SF', 25, 72],
      ['Cam Whitmore', 'SF', 22, 72],
      ['Julian Strawther', 'PG', 24, 71],
    ]},
    { name: 'Detroit Pistons', abbr: 'DET', players: [
      ['Cade Cunningham', 'PG', 24, 90],
      ['Jalen Duren', 'C', 22, 82],
      ['Ausar Thompson', 'PG', 23, 79],
      ['John Collins', 'SF', 28, 78],
      ['Javonte Green', 'PG', 33, 74],
      ['Gary Harris', 'PG', 32, 74],
      ['Kevin Huerter', 'PG', 28, 74],
      ['Paul Reed', 'SF', 27, 74],
      ['Duncan Robinson', 'SF', 32, 74],
      ['Isaiah Joe', 'PG', 27, 73],
      ['Wendell Moore Jr.', 'SF', 25, 72],
      ['Taurean Prince', 'SF', 32, 72],
    ]},
    { name: 'Golden State Warriors', abbr: 'GS', players: [
      ['Stephen Curry', 'PG', 38, 91],
      ['Kristaps Porzingis', 'C', 31, 84],
      ['Draymond Green', 'SF', 36, 78],
      ['Brandin Podziemski', 'PG', 23, 77],
      ['Charles Bassey', 'C', 25, 74],
      ['De\'Anthony Melton', 'PG', 28, 74],
      ['Moses Moody', 'PG', 24, 74],
      ['Al Horford', 'C', 40, 73],
      ['Georges Niang', 'SF', 33, 73],
      ['Gary Payton II', 'PG', 33, 73],
      ['Nate Williams', 'PG', 27, 72],
      ['Gui Santos', 'SF', 24, 71],
    ]},
    { name: 'Houston Rockets', abbr: 'HOU', players: [
      ['Kevin Durant', 'SF', 37, 90],
      ['Alperen Sengun', 'C', 24, 87],
      ['Amen Thompson', 'PG', 23, 84],
      ['Jabari Smith Jr.', 'SF', 23, 79],
      ['Fred VanVleet', 'PG', 32, 78],
      ['Tari Eason', 'SF', 25, 77],
      ['Steven Adams', 'C', 33, 75],
      ['Bogdan Bogdanovic', 'PG', 34, 75],
      ['Reed Sheppard', 'PG', 22, 75],
      ['Clint Capela', 'C', 32, 74],
      ['Marcus Smart', 'PG', 32, 74],
      ['Jae\'Sean Tate', 'SF', 30, 74],
    ]},
    { name: 'Indiana Pacers', abbr: 'IND', players: [
      ['Tyrese Haliburton', 'PG', 26, 90],
      ['Pascal Siakam', 'SF', 32, 86],
      ['Ivica Zubac', 'C', 29, 83],
      ['Andrew Nembhard', 'PG', 26, 78],
      ['Aaron Nesmith', 'PG', 26, 77],
      ['Obi Toppin', 'SF', 28, 75],
      ['Jay Huff', 'C', 29, 74],
      ['T.J. McConnell', 'PG', 34, 74],
      ['Kelly Oubre Jr.', 'PG', 30, 74],
      ['Jarace Walker', 'SF', 23, 74],
      ['Larry Nance Jr.', 'SF', 33, 73],
      ['Quenton Jackson', 'PG', 28, 72],
    ]},
    { name: 'LA Clippers', abbr: 'LAC', players: [
      ['Darius Garland', 'PG', 26, 86],
      ['Brandon Ingram', 'SF', 29, 83],
      ['Rui Hachimura', 'SF', 28, 76],
      ['Max Strus', 'PG', 30, 76],
      ['Gradey Dick', 'PG', 22, 74],
      ['Derrick Jones Jr.', 'SF', 29, 74],
      ['Brook Lopez', 'C', 38, 74],
      ['Bradley Beal', 'PG', 33, 73],
      ['Kris Dunn', 'PG', 32, 72],
      ['Isaiah Jackson', 'SF', 24, 72],
      ['TyTy Washington Jr.', 'PG', 24, 72],
      ['Jordan Miller', 'PG', 26, 71],
    ]},
    { name: 'Los Angeles Lakers', abbr: 'LAL', players: [
      ['Luka Doncic', 'PG', 27, 96],
      ['Austin Reaves', 'PG', 28, 84],
      ['Walker Kessler', 'C', 25, 79],
      ['Collin Sexton', 'PG', 27, 78],
      ['Quentin Grimes', 'PG', 26, 76],
      ['Kevon Looney', 'SF', 30, 74],
      ['Sandro Mamukelashvili', 'SF', 27, 74],
      ['Matisse Thybulle', 'PG', 29, 74],
      ['Dalton Knecht', 'SF', 25, 73],
      ['Jarred Vanderbilt', 'SF', 27, 73],
      ['Ziaire Williams', 'SF', 25, 73],
      ['Jaden Hardy', 'PG', 24, 72],
    ]},
    { name: 'Memphis Grizzlies', abbr: 'MEM', players: [
      ['Zach Edey', 'C', 24, 78],
      ['Ty Jerome', 'PG', 29, 76],
      ['D\'Angelo Russell', 'PG', 30, 76],
      ['Jerami Grant', 'SF', 32, 75],
      ['Isaiah Stewart', 'SF', 25, 75],
      ['Jaylen Wells', 'SF', 23, 75],
      ['Taylor Hendricks', 'SF', 22, 73],
      ['Cedric Coward', 'SF', 23, 72],
      ['Scotty Pippen Jr.', 'PG', 25, 72],
      ['Jordan Hawkins', 'PG', 24, 71],
      ['GG Jackson', 'SF', 21, 71],
      ['Kris Murray', 'SF', 26, 71],
    ]},
    { name: 'Miami Heat', abbr: 'MIA', players: [
      ['Giannis Antetokounmpo', 'SF', 31, 96],
      ['Bam Adebayo', 'C', 29, 86],
      ['Klay Thompson', 'PG', 36, 78],
      ['Andrew Wiggins', 'SF', 31, 78],
      ['Bobby Portis', 'SF', 31, 76],
      ['Tim Hardaway Jr.', 'PG', 34, 74],
      ['Nikola Jovic', 'SF', 23, 74],
      ['Davion Mitchell', 'PG', 28, 74],
      ['Nick Richards', 'C', 28, 74],
      ['Simone Fontecchio', 'SF', 30, 72],
      ['Dru Smith', 'PG', 28, 72],
      ['Keshad Johnson', 'SF', 25, 69],
    ]},
    { name: 'Milwaukee Bucks', abbr: 'MIL', players: [
      ['Tyler Herro', 'PG', 26, 84],
      ['Myles Turner', 'C', 30, 80],
      ['Kel\'el Ware', 'C', 22, 78],
      ['Kevin Porter Jr.', 'PG', 26, 76],
      ['Caris LeVert', 'PG', 32, 75],
      ['Gary Trent Jr.', 'PG', 27, 75],
      ['Kyle Kuzma', 'SF', 31, 74],
      ['Jericho Sims', 'C', 27, 74],
      ['AJ Green', 'PG', 26, 73],
      ['Ousmane Dieng', 'SF', 23, 72],
      ['Ryan Rollins', 'PG', 24, 72],
      ['Jaime Jaquez Jr.', 'SF', 25, 71],
    ]},
    { name: 'Minnesota Timberwolves', abbr: 'MIN', players: [
      ['Anthony Edwards', 'PG', 25, 93],
      ['LaMelo Ball', 'PG', 25, 86],
      ['Rudy Gobert', 'C', 34, 82],
      ['Jaden McDaniels', 'SF', 25, 80],
      ['Donte DiVincenzo', 'PG', 29, 76],
      ['Ayo Dosunmu', 'PG', 26, 76],
      ['Bones Hyland', 'PG', 26, 74],
      ['John Konchar', 'PG', 30, 74],
      ['Trey Lyles', 'SF', 30, 74],
      ['Terrence Shannon Jr.', 'PG', 26, 73],
      ['Jonathan Kuminga', 'SF', 23, 72],
      ['Julian Phillips', 'SF', 22, 71],
    ]},
    { name: 'New Orleans Pelicans', abbr: 'NO', players: [
      ['Zion Williamson', 'SF', 26, 86],
      ['Trey Murphy III', 'SF', 26, 81],
      ['Dejounte Murray', 'PG', 30, 80],
      ['Bennedict Mathurin', 'PG', 24, 79],
      ['Jordan Poole', 'PG', 27, 78],
      ['Yves Missi', 'C', 22, 75],
      ['Saddiq Bey', 'PG', 27, 74],
      ['Herbert Jones', 'SF', 27, 74],
      ['Derik Queen', 'C', 21, 74],
      ['Jeremiah Fears', 'PG', 19, 72],
      ['Bryce McGowens', 'PG', 23, 72],
      ['Trendon Watford', 'SF', 25, 71],
    ]},
    { name: 'New York Knicks', abbr: 'NY', players: [
      ['Jalen Brunson', 'PG', 30, 91],
      ['Karl-Anthony Towns', 'C', 30, 88],
      ['OG Anunoby', 'SF', 29, 82],
      ['Mikal Bridges', 'PG', 30, 81],
      ['Josh Hart', 'PG', 31, 78],
      ['Jose Alvarado', 'PG', 28, 74],
      ['Miles McBride', 'PG', 26, 74],
      ['Landry Shamet', 'PG', 29, 74],
      ['Jordan Clarkson', 'PG', 34, 73],
      ['Andre Drummond', 'C', 33, 72],
      ['Pacome Dadiet', 'SF', 21, 69],
      ['Tyler Kolek', 'PG', 25, 69],
    ]},
    { name: 'Oklahoma City Thunder', abbr: 'OKC', players: [
      ['Shai Gilgeous-Alexander', 'PG', 28, 97],
      ['Jalen Williams', 'PG', 25, 88],
      ['Chet Holmgren', 'C', 24, 86],
      ['Isaiah Hartenstein', 'C', 28, 79],
      ['Alex Caruso', 'PG', 32, 76],
      ['Jared McCain', 'PG', 22, 76],
      ['Cason Wallace', 'PG', 22, 76],
      ['Kenrich Williams', 'PG', 31, 74],
      ['Jaylin Williams', 'SF', 24, 72],
      ['Ajay Mitchell', 'PG', 24, 69],
      ['Brooks Barnhizer', 'SF', 24, 68],
      ['Nikola Topic', 'PG', 21, 68],
    ]},
    { name: 'Orlando Magic', abbr: 'ORL', players: [
      ['Paolo Banchero', 'SF', 23, 88],
      ['Franz Wagner', 'SF', 25, 86],
      ['Desmond Bane', 'PG', 28, 84],
      ['Nikola Vucevic', 'C', 35, 80],
      ['Jalen Suggs', 'PG', 25, 79],
      ['Wendell Carter Jr.', 'C', 27, 76],
      ['Anthony Black', 'PG', 22, 74],
      ['Jevon Carter', 'PG', 31, 74],
      ['Goga Bitadze', 'C', 27, 73],
      ['Jonathan Isaac', 'SF', 28, 73],
      ['Malaki Branham', 'SF', 23, 72],
      ['Jamal Cain', 'SF', 27, 72],
    ]},
    { name: 'Philadelphia 76ers', abbr: 'PHI', players: [
      ['Jaylen Brown', 'PG', 29, 90],
      ['Joel Embiid', 'C', 32, 90],
      ['Tyrese Maxey', 'PG', 25, 89],
      ['VJ Edgecombe', 'PG', 21, 78],
      ['Kentavious Caldwell-Pope', 'PG', 33, 74],
      ['Anfernee Simons', 'PG', 27, 74],
      ['Dominick Barlow', 'SF', 23, 72],
      ['Jabari Walker', 'SF', 24, 72],
      ['Tacko Fall', 'C', 30, 71],
      ['Rayan Rupert', 'PG', 22, 71],
      ['Dean Wade', 'SF', 29, 71],
      ['Adem Bona', 'C', 23, 69],
    ]},
    { name: 'Phoenix Suns', abbr: 'PHX', players: [
      ['Devin Booker', 'PG', 29, 90],
      ['Jalen Green', 'PG', 24, 81],
      ['Miles Bridges', 'SF', 28, 80],
      ['Mark Williams', 'C', 24, 77],
      ['Dillon Brooks', 'SF', 30, 76],
      ['Luke Kennard', 'PG', 30, 75],
      ['Jordan Goodwin', 'PG', 27, 74],
      ['Haywood Highsmith', 'SF', 29, 74],
      ['Ryan Dunn', 'SF', 23, 73],
      ['Jamaree Bouyea', 'PG', 27, 72],
      ['Isaiah Livers', 'SF', 28, 72],
      ['Khaman Maluach', 'C', 20, 72],
    ]},
    { name: 'Portland Trail Blazers', abbr: 'POR', players: [
      ['Ja Morant', 'PG', 27, 88],
      ['Deni Avdija', 'SF', 25, 82],
      ['Damian Lillard', 'PG', 36, 82],
      ['Shaedon Sharpe', 'PG', 23, 80],
      ['Jrue Holiday', 'PG', 36, 78],
      ['Scoot Henderson', 'PG', 22, 77],
      ['Toumani Camara', 'SF', 26, 76],
      ['Donovan Clingan', 'C', 22, 76],
      ['Robert Williams III', 'C', 28, 74],
      ['Vit Krejci', 'PG', 26, 72],
      ['Micah Potter', 'C', 28, 72],
      ['Jeremy Sochan', 'SF', 23, 72],
    ]},
    { name: 'Sacramento Kings', abbr: 'SAC', players: [
      ['Domantas Sabonis', 'SF', 30, 85],
      ['Zach LaVine', 'PG', 31, 83],
      ['De\'Andre Hunter', 'SF', 28, 78],
      ['Malik Monk', 'PG', 28, 77],
      ['Keegan Murray', 'SF', 26, 76],
      ['Precious Achiuwa', 'PF', 27, 74],
      ['Nique Clifford', 'PG', 24, 71],
      ['Adam Flagler', 'PG', 26, 69],
      ['Jonathan Mogbo', 'SF', 24, 69],
      ['Daeqwon Plowden', 'PG', 28, 69],
      ['Dylan Cardwell', 'C', 24, 68],
      ['Maxime Raynaud', 'C', 23, 68],
    ]},
    { name: 'San Antonio Spurs', abbr: 'SA', players: [
      ['Victor Wembanyama', 'SF', 22, 94],
      ['De\'Aaron Fox', 'PG', 28, 87],
      ['Stephon Castle', 'PG', 21, 80],
      ['Dylan Harper', 'PG', 20, 78],
      ['Devin Vassell', 'PG', 26, 78],
      ['Tobias Harris', 'SF', 34, 76],
      ['Keldon Johnson', 'SF', 26, 74],
      ['Jordan McLaughlin', 'PG', 30, 74],
      ['Luke Kornet', 'C', 31, 73],
      ['Harrison Barnes', 'SF', 34, 72],
      ['Julian Champagnie', 'SF', 25, 72],
      ['Harrison Ingram', 'SF', 23, 69],
    ]},
    { name: 'Toronto Raptors', abbr: 'TOR', players: [
      ['Kawhi Leonard', 'SF', 35, 86],
      ['Scottie Barnes', 'SF', 25, 85],
      ['RJ Barrett', 'SF', 26, 79],
      ['Jakob Poeltl', 'C', 30, 78],
      ['Immanuel Quickley', 'PG', 27, 78],
      ['Kyle Anderson', 'SF', 33, 74],
      ['A.J. Lawson', 'PG', 26, 72],
      ['Andre Jackson Jr.', 'PG', 24, 71],
      ['Trayce Jackson-Davis', 'SF', 26, 71],
      ['Trey Jemison III', 'C', 26, 71],
      ['Collin Murray-Boyles', 'SF', 21, 71],
      ['Jamison Battle', 'SF', 25, 69],
    ]},
    { name: 'Utah Jazz', abbr: 'UTAH', players: [
      ['Jaren Jackson Jr.', 'SF', 27, 86],
      ['Lauri Markkanen', 'SF', 29, 84],
      ['Keyonte George', 'PG', 22, 76],
      ['Ace Bailey', 'PG', 20, 75],
      ['Mo Bamba', 'C', 28, 74],
      ['Isaiah Collier', 'PG', 21, 74],
      ['Josh Green', 'PG', 25, 74],
      ['Jaxson Hayes', 'C', 26, 74],
      ['Svi Mykhailiuk', 'PG', 29, 74],
      ['Josh Okogie', 'PG', 28, 74],
      ['Kyle Filipowski', 'SF', 22, 73],
      ['Jusuf Nurkic', 'C', 32, 73],
    ]},
    { name: 'Washington Wizards', abbr: 'WSH', players: [
      ['Anthony Davis', 'SF', 33, 89],
      ['Trae Young', 'PG', 28, 89],
      ['Deandre Ayton', 'C', 28, 79],
      ['Alex Sarr', 'C', 21, 77],
      ['Justin Champagnie', 'SF', 25, 74],
      ['Bilal Coulibaly', 'PG', 22, 74],
      ['Tre Johnson', 'PG', 20, 74],
      ['Tre Mann', 'PG', 25, 74],
      ['Bub Carrington', 'PG', 21, 73],
      ['Kyshawn George', 'SF', 22, 73],
      ['Anthony Gill', 'SF', 33, 73],
      ['Khris Middleton', 'SF', 35, 73],
    ]},
  ],
};
